...
def walk(
    ...
    declared = 0
    built_with = -1
    # Every variable leaf is *declared* to the horizon and *admitted* as the walk
    # reaches the first theorem that may mention it. The leaves are what a rebuild
    # cannot scope on its own - they change with every statement, not only with
    # notation - so they are grown instead, which is what keeps the walk's scope
    # equal to `import_theorem`'s rather than weaker.
    ...
    for label in database.order[: database.position(horizon) + 1]:
        ...
        if label in wanted:
            ...
            admitted = _admit_variables(
                system, schedule, admitted, database.position(label)
            )
...
def _admit_variables(
    system: FormalSystem, schedule: VariableSchedule, admitted: int, position: int
) -> int:
    # Admit every variable leaf scheduled at or before `position` and not already
    # in, and report the watermark to resume from. `admitted` of 0 means a system
    # fresh from `build_spec`, whose sorts hold every leaf declared to the
    # horizon - so empty the sub-sorts first and let the schedule refill them.
    #
    # Only the sub-sorts are emptied. Their inclusion into a typecode is left
    # standing, since an empty union admits nothing and removing a member from a
    # live sort would take the notation with it.
    context = system.build_context
    if admitted == 0:
        for sub_sort in schedule.sub_sorts:
            context.variables[sub_sort].clear_patterns()
