...
**Status: this note is the deliverable. The code that produced it is a spike and
is not maintained.** It lived on a throwaway branch under `experiments/`, was
never imported by `app/` or `website/`, and was never read by the test suite. If
any of it is pursued, §8 says which parts are worth rebuilding properly and which
were scaffolding — but nothing below depends on that code still existing, and the
numbers are recorded here rather than only in the runs that produced them.
